 ...
 * @property {LayoutRenderFn} layout
 ...
 * @type {BuildPage}
 */
const buildPage = async ({page, layout, output}) => {
  ...
  const pageContent = await page(templateHelpers);
  const html = await layout(pageContent, templateHelpers);
