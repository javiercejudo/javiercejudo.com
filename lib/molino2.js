 ...
 * @template Data
 * @typedef Page
 * @property {RenderFn<Data>} renderFn
 * @property {PageDataMapper<Data>} dataMapper
 */
 ...
 * @typedef Layout
 * @property {RenderFn<Data>} renderFn
 * @property {LayoutDataMapper<Data>} dataMapper
 */
...
/**
 * @template PageData, LayoutData
 * @typedef BuildPageInput
 * @property {Layout<LayoutData>} layout
 * @property {Page<PageData>} page
 ...
 * @template PageData, LayoutData
 * @callback BuildPage
 * @param {BuildPageInput<PageData, LayoutData>} input
 * @returns {Promise<BuildPageOutput>}
 */

/**
 * @template PageData, LayoutData
 * @type {BuildPage<PageData, LayoutData>}
 */
const buildPage = async ({layout, page, output}) => {
  ...
  const pageData = await page.dataMapper(templateHelpers);
  const pageContent = await page.renderFn(pageData);
  const layoutData = await layout.dataMapper(pageContent, templateHelpers);
  const html = await layout.renderFn(layoutData);
