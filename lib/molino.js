...
/**
 * @typedef BuildPageContentProps
 * @property {string} sourcePath
 * @property {(template: string) => string} render
 */

/**
 * @callback BuildPageContent
 * @param {BuildPageContentProps} props
 * @returns {Promise<string>}
 */

/** @type BuildPageContent */
const buildPageContent = async ({sourcePath, render}) =>
  fsp.readFile(sourcePath).then(page => render(page.toString()));

/**
 * @typedef BuildHtmlProps
 * @property {string} sourcePath
...
/**
 * @callback BuildHtml
 * @param {BuildHtmlProps} props
 * @returns {Promise<string>}
 */

/** @type BuildHtml */
const buildHtml = async ({sourcePath, outputPath, render}) => {
...
const buildPage = async ({
  ...
  const pageContent = await buildPageContent({
  ...
  const html = await buildHtml({
    ...
    outputPath,
    render: layout =>
      renderLayout(layout, layoutData(pageContent, templateHelpers)),
  });
